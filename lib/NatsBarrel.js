	...
	return self.innerCreateIn( division, DIVISION_REPORTS, '', async function ( message ) {
		try {
			...
			if ( !self.presences[ status.division ][ status.entity ] ) {
				...
				await self.igniteSystemEvent( 'entityAppeared', status.division, status.context || '', status.entity ).catch( self.logger.harconlog )
			}

			self.presences[ status.division ][ status.entity ][ status.nodeID ] = {
				timestamp: Date.now(), warper: self.warper.inpose( status.warper ),
				projectVersion: status.projectVersion, entityVersion: status.entityVersion
			}
